...
#ifndef EXAMPLES_VYD_APPS_VYD_ARROW_PROJECTED_FRAGMENT_H_
#define EXAMPLES_VYD_APPS_VYD_ARROW_PROJECTED_FRAGMENT_H_
...
#include "context_protocols.h"
#include "arrow_projected_vertex_map.h"
...
class ArrowProjectedFragment
 ...
 public:
  ...
  Project(std::shared_ptr<vineyard::ArrowFragment<oid_t, vid_t>> fragment,
    ...
    LOG(INFO) << "ArrowProjectedFragment ObjectID:" << vineyard::ObjectIDToString(id);
  ...
  void Construct(const vineyard::ObjectMeta& meta) override {
    ...
    if (ivnum_ > 0) {
      ienum_ = static_cast<size_t>(oe_offsets_end_->Value(ivnum_-1) - oe_offsets_begin_->Value(0));
      if (directed_) {
        ienum_ = static_cast<size_t>(ie_offsets_end_->Value(ivnum_-1) - ie_offsets_begin_->Value(0));
      }
    }
    if(ovnum_ > 0) {
      oenum_ = static_cast<size_t>(oe_offsets_end_->Value(tvnum_-1) - oe_offsets_begin_->Value(ivnum_));
      if(directed_) {
        oenum_ = static_cast<size_t>(ie_offsets_end_->Value(tvnum_-1) - ie_offsets_begin_->Value(ivnum_));
      }
    }
  ...
  std::string dummy() const {return std::string("hello");}
...
#endif  // EXAMPLES_VYD_APPS_VYD_ARROW_PROJECTED_FRAGMENT_H_
